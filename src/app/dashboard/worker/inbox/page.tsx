...
export default async function WorkerInboxPage() {
  ...
  const threads = await getInboxThreads(user.id, "WORKER");

  return (
    <div>
      <h1 className="text-xl font-bold text-gray-900">
        Inbox <span className="font-normal text-gray-400">इनबॉक्स</span>
      </h1>
      <p className="mt-1 text-sm text-gray-500">Chats with recruiters about your bookings.</p>
      ...
      {threads.length === 0 ? (
        <div className="mt-6 rounded-xl border border-gray-200 bg-white p-8 text-center">
          ...
          <p className="mt-0.5 text-xs text-gray-400">
            Chats appear here once you have a booking with a recruiter.
          </p>
        </div>
      ) : (
